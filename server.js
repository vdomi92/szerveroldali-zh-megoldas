...
fastify.decorate("auth", async function (request, reply) {
    try {
        ...
        reply.code(401).send({ error: "UNAUTHORIZED", message: err });
    }
});

fastify.decorate("isPm", async function (request, reply) {
    const userEmail = request.user.email;

    // Ellenőrizd, hogy a felhasználó project manager-e
    if (userEmail !== "projectmanager@example.com") {
        reply.code(403).send({ error: "Forbidden", message: "User is not a project manager." });
        return;
    }
});
